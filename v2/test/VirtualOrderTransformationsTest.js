const VirtualOrder = require("../../v1/model/VitrualOrder").Constructor;
const Timeslot = require("../../v1/model/Timeslot");
const Parameter = require("../../v1/model/Parameter");
const RepositoryContextFactory = require("../factory/RepositoryContextFactory");
const TableGatewayContextFactory = require("../factory/TableGatewayContextFactory");
const InsertVirtualOrderContextFactory = require("../factory/InsertVirtualOrderContextFactory");
const Environment = require("../../v1/model/Environment").Constructor;
const BatchExecutorBuilder = require("../../basic/BatchExecutor").Builder;
...
module.exports = ()=>{
    QUnit.module("virtual-order-transformation", {
        before: ()=>{
            ...
            const repositoryContextFactory = new RepositoryContextFactory();
            const tableGatewayContextFactory = new TableGatewayContextFactory();
            const sqliteActionContextFactory = new InsertVirtualOrderContextFactory();
            this.useCaseContext = useCaseContext;
            this.repositoryContext = repositoryContextFactory.create(useCaseContext);
            this.tableGatewayContext = tableGatewayContextFactory.create(this.repositoryContext);
            this.sqliteActionContext = sqliteActionContextFactory.create(this.tableGatewayContext);
            this.factoryChain = new BatchExecutorBuilder().addMethod("create").chain(true)
                .addExecutor(repositoryContextFactory)
                .addExecutor(tableGatewayContextFactory)
                .addExecutor(sqliteActionContextFactory)
                .build();
        }
    });
    QUnit.test("VirtualOrder => VirtualOrderDTO", assert =>{
        const firstOrderDTO = this.repositoryContext.models[0].dto;
    ...
    QUnit.test("VirtualOrderDTO => TablesDTO[]", assert =>{
        // console.log(this.tableGatewayContext.tables);
        const voTable = this.tableGatewayContext.tables[0];
        assert.strictEqual(voTable.name, "Virtual Orders")
        assert.strictEqual(voTable.columns.join("|"), "Id|Symbol|Environment When Sent|Environment When Executed|Exec Delay");
        assert.strictEqual(voTable.records.length, 2);
        const firstVoRecord = voTable.records[0];
    ...
    QUnit.test("TablesDTO[] => SQL Statements", assert=>{
        ...
        const statements = this.sqliteActionContext.statements;
        assert.strictEqual(statements.length, 6);
        assert.strictEqual(statements[0], "insert into virtual_orders (id, symbol, sent_environment_id, executed_environment_id, exec_delay) values ('vo-1','MSFT','env-sent-1','env-exec-1',400)");
        assert.strictEqual(statements[2], "insert into parameters (id, name, value, parent_id) values ('buy-vo-1','buy',-100,'vo-1')")
    });
    QUnit.test("Virtual Orders => SQL Statements", assert=>{
        const sqliteActionContext = this.factoryChain.execute(this.useCaseContext);
        const statements = sqliteActionContext.statements;
        assert.strictEqual(statements.length, 6);
        assert.strictEqual(statements[0], "insert into virtual_orders (id, symbol, sent_environment_id, executed_environment_id, exec_delay) values ('vo-1','MSFT','env-sent-1','env-exec-1',400)");
        assert.strictEqual(statements[2], "insert into parameters (id, name, value, parent_id) values ('buy-vo-1','buy',-100,'vo-1')")
    });
